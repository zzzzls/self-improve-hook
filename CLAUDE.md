...
skills/self-improve/          # 流水线 skill —— 斜杠命令 /self-improve(init/update/destroy)
  ...
  manager.py                  # 编排器:init 启用(hook+deny 保护) / update 仅合并 / destroy 拆 hook+deny
  scripts/                    # 流水线脚本
...
- 查看 LLM 实际调用:目标项目 `.claude/memory/runs/<时间戳>-*/` 下的 `prompt.txt` / `response.txt` / `meta.json`。`meta.json` 记 `candidates_raw/appended/rejected` 统计;**当提取结果为空数组**时还会记 `empty_reason`(LLM 给出的"为何本段没有可沉淀内容",同时也进 `hook.log`),便于调试为何没提取到。
...
- **runlog 的 `rec.extras` 必须在 `with` 块内写**:`runlog.record_run` 在 `with` 退出时(`finally`)即 `_persist` 落盘 `meta.json`,**块外再改 `rec.extras` 不会被写入**。曾因此把 `candidates_raw/appended/rejected` 写在块外导致它们从未进过 meta.json。现已把候选 enrich/append 与所有 extras 赋值统一收进 `run_extraction` 的 `with` 块内。提取 prompt(`extract.md`)要求 LLM 在 `{"candidates": []}` 为空时附带 `reason` 字段,代码取到后写入 `rec.extras["empty_reason"]` 便于 debug。
...
- **启用/禁用走 `/self-improve` skill**:推荐 `/self-improve init` 在目标项目启用(把 hook 写进该项目个人 `.claude/settings.local.json`,不入库)、`/self-improve update` 手动合并、`/self-improve destroy` 移除(拆 hook + deny 规则,保留 `.claude/memory/` 与 `CLAUDE.md` 引用)。
- **防主会话误改记忆(deny 规则 + 托管块说明)**:主会话 Claude 可能擅自编辑 `memory.generated.md` 或 `.claude/memory/` 内部文件,破坏流水线。`init` 在 `settings.local.json` 注入 `permissions.deny`(见 `PERMISSION_DENY_RULES`)+ 在 CLAUDE.md 托管块写入禁改说明(见 `MANAGED_BLOCK_BODY`)双管齐下;`destroy` 对称移除 deny 规则(`_remove_deny_rules`)。**关键权衡**:`memory.generated.md` 只禁 `Edit`/`Write`、**保留 `Read`** —— deny `Read` 是硬阻断且会一并切断 CLAUDE.md 的 `@memory.generated.md` 回注(@ 引用走 Read 权限),禁读等于让记忆喂不回会话、核心功能作废;`.claude/memory/**`(候选/state/runs/archive)不走 @ 注入,故连读带写全禁。流水线自身写 `memory.generated.md` 走 Python `os.replace`、不经 claude 工具权限,**不受 deny 影响**。托管块 `_init_claude_md` 现按 `begin/end` 标记**整体替换升级**(旧的纯 `@` 块重跑 `init` 会被升级为带保护说明的新块)。
...
- **stdin/子进程 IO 一律钉死 UTF-8(Windows 必需)**:Windows 中文区域默认 ANSI 代码页(cp936/GBK),`json.load(sys.stdin)` 会用 cp936 解码 Claude Code 以 UTF-8 传入的负载——含中文的 `cwd` 被解成乱码,于是 `memory_dir` 指向**幽灵目录**,日志/状态/候选全写错地方(真实目录空着,看起来"hook 没触发",实则一直在跑)。三个入口脚本统一改为 `json.loads(sys.stdin.buffer.read().decode("utf-8"))` 并兼容捕获 `UnicodeDecodeError`;配套地,喂 `consolidate` stdin 的写入端(`manager.cmd_update` / `session_start._trigger_consolidate`)都加 `encoding="utf-8"`,读写两端一致。`lib/claude_cli.py` 调 `claude` 也 `encoding="utf-8"`,且 **prompt 改走 stdin**(`-p` 不带位置参数、`input=prompt`)而非命令行参数——既避免 Windows 上经 `cmd /c` 时被 shell 引号/编码弄坏或撞命令行长度上限(症状:`claude -p` 返回空),也保证中文 prompt 不乱码。
